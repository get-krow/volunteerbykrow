  ...
  return (
    <div className={`flex flex-col items-center select-none ${className}`}>
      ...
      <div className="relative w-full h-[160px] bg-slate-50/90 rounded-2xl border border-gray-200/90 shadow-2xs overflow-hidden">
        {/* Apple Center Highlight Band */}
        <div className="absolute inset-x-1.5 top-1/2 -translate-y-1/2 h-[40px] bg-white border-y-2 border-[#635BFF]/50 rounded-xl shadow-xs pointer-events-none z-10" />

        {/* Top/Bottom 3D Perspective Fades */}
        <div className="absolute inset-x-0 top-0 h-[60px] bg-gradient-to-b from-slate-50 via-slate-50/80 to-transparent z-20 pointer-events-none" />
        <div className="absolute inset-x-0 bottom-0 h-[60px] bg-gradient-to-t from-slate-50 via-slate-50/80 to-transparent z-20 pointer-events-none" />

        {/* Scroll Container */}
        <div
          ref={containerRef}
          onScroll={handleScroll}
          className="h-full w-full overflow-y-auto snap-y snap-mandatory scrollbar-none py-[60px]"
          ...
          {options.map((opt) => {
            ...
            return (
              <div
                ...
                className={`h-[40px] flex items-center justify-center snap-center cursor-pointer transition-all duration-150 text-xs px-1 ${
                  isSelected
                    ? 'font-black text-[#635BFF] scale-105'
