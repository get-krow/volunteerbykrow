...
export default function OrganizerOpportunitiesPage() {
  ...
  return (
    <MainLayout>
      {currentUser ? (
        <OrganizerPortal initialTab="opportunities" currentUser={currentUser} onLogout={() => { db.setCurrentUser(null); setCurrentUser(null); window.location.href = '/'; }} />
      ) : (
        <div className="max-w-md mx-auto my-20 p-8 bg-white rounded-3xl border border-gray-200 shadow-sm text-center">
          ...
          <p className="text-xs text-gray-500 font-medium">Please sign in to manage your organization's opportunities.</p>
        </div>
      )}
    </MainLayout>
  );
