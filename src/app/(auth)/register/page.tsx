...
function RegisterForm() {
  ...
  // Volunteer Onboarding Form State
  ...
  // Dynamic region & city lists based on selected country & province
  ...
  const handleNextStep = (e: React.FormEvent) => {
    ...
    if (selectedRole === "volunteer") {
      ...
      // Direct submit for organization
  ...
  const handleCompleteRegister = async () => {
    ...
    if (selectedRole === "volunteer") {
      ...
      formData.set("country", country === "CA" ? "Canada" : "United States");
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">
            {step === 1 ? "Create an account" : "Personal Profile & Location"}
          </h2>
          <p className="mt-1 text-xs text-muted-foreground">
            {step === 1
              ? "Join Krow to discover opportunities or recruit volunteers"
              : "Tell us a bit about yourself so we can match local events"}
          </p>
        </div>
        ...
        <>
          ...
          {selectedRole === "volunteer" && (
            <>
              <form action={handleGoogleSignIn} className="mt-6">
                <Button type="submit" variant="outline" className="w-full h-11 gap-2 font-medium">
          ...
          <form onSubmit={handleNextStep} className="space-y-4 mt-4">
            ...
            <Button type="submit" className="w-full h-11 font-semibold" disabled={isLoading}>
              ...
              {selectedRole === "volunteer" ? "Next: Personal Profile & Location →" : "Create Organization Account"}
            </Button>
          </form>
        </>
      )}
      ...
      {step === 2 && selectedRole === "volunteer" && (
        <form onSubmit={(e) => { e.preventDefault(); handleCompleteRegister(); }} className="space-y-4 mt-6">
          {error && (
            <div className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">
              {error}
            </div>
          ...
          {/* City Dropdown (Populated by region) */}
          <div className="space-y-2">
            <Label>City *</Label>
          ...
          <div className="flex gap-3 pt-2">
            <Button type="button" variant="outline" onClick={() => setStep(1)} className="h-11 flex-1 font-semibold">
              ← Back
            </Button>
            <Button type="submit" className="h-11 flex-1 font-semibold" disabled={isLoading}>
              {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
      ...
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center bg-gradient-to-br from-primary via-primary/90 to-accent p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md">
          ...
          <p className="mt-4 text-lg text-white/70 leading-relaxed">
            Create your free account and start making a positive impact in your
            community today.
          </p>
        </div>
      </div>
